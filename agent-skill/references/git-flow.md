...
- **Pre-push hook chạy `npm run build`, yêu cầu Node ≥20.19** (không phải ≥20.9 — `vite`/`vitest` cần `node:util`'s `styleText`, chỉ có từ Node 20.12 trở lên; CI's `compat-node-min` job từng bắt được đúng lỗi này khi engines field ghi sai 20.9.0) — máy có thể đang mặc định Node 18 (`nvm`), khiến `git push`/`git commit` fail ở bước husky. Luôn `nvm use 22` (hoặc bất kỳ bản ≥20.19 có sẵn) trước khi commit/push.
...
- **Chưa bật branch protection trên GitHub** — hiện tại ai có quyền push đều bấm "Merge pull request" được luôn, không bắt buộc review hay status check pass trước. Cần vào GitHub → Settings → Branches → bật "Require pull request reviews before merging" + "Require status checks to pass" (chọn check `build-and-test` từ workflow CI bên dưới) cho cả `main` và `develop`. Việc này phải làm trên GitHub UI, ngoài khả năng thao tác trực tiếp của agent — chỉ có thể nhắc, không tự bật được.
- **Merge code cục bộ (`git merge` + `git push`) không tạo "Compare & pull request" trên GitHub, và trước khi có CI thì cũng không có build check nào chạy trên GitHub** — chỉ có Husky hook (lint/test/build) chạy local. Từ khi có `.github/workflows/ci.yml`, mọi `push`/`pull_request` nhắm `main`/`develop` đều tự chạy lint + tsc + test + build trên GitHub Actions, kể cả khi merge local rồi push thẳng (không qua PR) — job vẫn chạy trên sự kiện `push`.
