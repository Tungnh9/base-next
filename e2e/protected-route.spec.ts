...
test.describe("Route protection", () => {
  ...
  test("logged-in non-admin visiting /settings (admin-only per src/config/nav.ts) redirects to /not-authorized", async ({
    page,
  }) => {
    await login(page, MOCK_NORMAL_EMAIL)
    await expect(page).toHaveURL(/\/en\/dashboard/)
  ...
  test("logged-in admin mock account can reach /settings", async ({ page }) => {
    await login(page, MOCK_ADMIN_EMAIL)
    await expect(page).toHaveURL(/\/en\/dashboard/)
